...
class Reader():
    ...
    def __init__(self, model_path, data_dir, logger, window_size, batch_size, has_dev=True, audio_encoder='hubert_base') -> None:
        ...
        fallback_encoded_data_path = "./encoded_data.pkl"
        if os.path.exists(encoded_data_path):
            load_path = encoded_data_path
        elif os.path.exists(fallback_encoded_data_path):
            load_path = fallback_encoded_data_path
        else:
            raise FileNotFoundError(
                "encoded_data.pkl not found. Checked: {} and {}".format(
                    encoded_data_path, fallback_encoded_data_path
                )
            )

        self.logger.info("Load encoded data from {}".format(load_path))
        self.data = load_pickle(load_path)
