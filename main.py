...
def main():
    ...
    mse_tag = "-no_mse" if cfg.no_mse else "-mse{}-pm{}-k{}".format(
        cfg.mse_weight, cfg.primary_modality, cfg.swd_proj_k)
    drp_tag = "-no_drp" if cfg.no_drp else "-drp{}".format(cfg.drp_weight)
    ...
    t5_config.setdefault('gat_config', {})
    t5_config['gat_config']['num_of_layers'] = cfg.gat
    ...
    if cfg.run_type == "train":
        ...
        cmd = "python3 main.py -run_type predict -ckpt {} -output predict_real.json -test_batch_size {}"
        best_ckpt = os.path.join(cfg.model_dir, "ckpt-best")
        if os.path.exists(best_ckpt):
            subprocess.run(cmd.format(best_ckpt, cfg.test_batch_size), shell=True)
        else:
